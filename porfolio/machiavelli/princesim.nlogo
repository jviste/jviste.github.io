...
to setup
  ...
  ;; create cops
  create-cops round (number_of_soldiers * .01 * count patches) [
    move-to one-of patches with [not any? turtles-here]
    display-cop
  ]

  create-princes round (1) [
    move-to one-of patches with [not any? turtles-here]
    display-prince
  ]

  ;; create agents
  create-agents round (number_of_citizens * .01 * count patches) [
    move-to one-of patches with [not any? turtles-here]
    set heading 0
...
to go
  ask turtles [
    ...
    if breed = agents and jail-term = 0 [ determine-behaviorA ]
...
to move ;; turtle procedure
  if movement? or (breed = cops) or (breed = princes) [
...
to determine-behaviorA
  set active? (grievance - risk-aversion * estimated-arrest-probability > threshold)
  set active? ((Generosity + Clemency) - Manipulate > threshold)
  if Generosity < 0.40 [
    set active? true
  ]
  if Generosity > 0.60 [
...
to-report grievance
  report perceived-hardship * (1 - Power_of_Prince)
end
...
523
279
Citizens Rebelling Against The Prince
Time
Citizens
0.0
20.0
...
"" ""
PENS
"Peaceful" 1.0 0 -10899396 true "" "plot count agents with [not active? and jail-term = 0]"
"Punished" 1.0 0 -16777216 true "" "plot count agents with [jail-term > 0]"
"Rebellion" 1.0 0 -2674135 true "" "plot count agents with [active?]"
